...
RSpec.describe EmployeesController, type: :controller do
  ...
  context 'not signed in user' do
    ...
    it 'should not have access to view employee information' do
      get :show, params: { company_name: @company.name, id: @employee.id }
      expect(response).to redirect_to(root_path)
      expect(controller).to set_flash[:alert]
    end

    it 'should not have access to edit personal employee information' do
      get :edit_information, params: { company_name: @company.name, id: @employee.id }
      expect(response).to redirect_to(root_path)
      expect(controller).to set_flash[:alert]
    end

    it 'should not have access to edit employee compensation' do
      get :edit_compensation, params: { company_name: @company.name, id: @employee.id }
      expect(response).to redirect_to(root_path)
      expect(controller).to set_flash[:alert]
    end
  end
  ...
  context 'signed in employee' do
    ...
    it 'should update personal information successfully' do
      put :update_information, params: { company_name: @company.name, id: @employee.id, employee: { address: '890 Old Street' } }
    ...
    it 'should not have access to view other employee information' do
      get :show, params: { company_name: @company.name, id: @employee2.id }
      expect(response).to redirect_to(root_path)
      expect(controller).to set_flash[:alert]
    end
    ...
    it 'should not be able to update compensation information' do
      put :update_compensation, params: { company_name: @company.name, id: @employee.id, employee: { salary: 100000 } }
      expect(response).to redirect_to(@company)
